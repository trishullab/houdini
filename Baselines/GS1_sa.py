...
from HOUDINI.Eval.EvaluatorUtils import get_io_examples_regress_speed_street_sign, \
    get_io_examples_shortest_path_street_sign_maze
...
def _train_s1t1(results_directory, save=True):
    train_io_examples, val_io_examples, test_io_examples = get_io_examples_regress_speed_street_sign()
...
def _train_s1t2(results_directory, save_name_cnn, save_name_mlp, save=True):
    if just_testing:
    ...
    nn_rnn_params = {"type": "RNN", "name": name_rnn, "input_dim": 2,
                     "hidden_dim": 101, "output_dim": 1, "output_activation": None, "output_sequence": True}

    new_fns_params = [nn_classifier_params1, nn_classifier_params2, nn_rnn_params]
    program = "lib.compose({}, lib.compose(lib.flatten_2d_list, lib.map_g(lib.compose({}, {}))))".format(name_rnn, name_mlp, name_cnn)
    # program = "lambda inputs: {}(lib.flatten_2d_list(lib.map_g(lib.compose({}, {}), inputs)))".format(name_rnn, name_mlp, name_cnn)
